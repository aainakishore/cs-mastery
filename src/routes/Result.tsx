        ...
        {passed ? (
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-slate-700 hover:bg-slate-600 text-slate-200 py-3 rounded-xl font-semibold"
            >
              🧠 Try Project
            </button>
            <button
              onClick={() => navigate('/')}
              className="w-full text-slate-400 hover:text-slate-200 py-2 text-sm"
            >
              Back to Home
            </button>
          </div>
            ...
            {failedTags.length > 0 && (
              <div className="bg-slate-800 rounded-xl p-3 text-left">
                ...
                <div className="flex flex-wrap gap-1">
                  {failedTags.map((tag) => (
                    <span key={tag} className="bg-red-900/50 text-red-300 text-xs px-2 py-0.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
